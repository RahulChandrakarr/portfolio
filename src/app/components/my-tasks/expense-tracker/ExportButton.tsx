...
export default function ExportButton({
  ...
  return (
    <button
      ...
      className="min-h-[44px] flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 active:bg-slate-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2"
      title="Export as CSV (Ctrl/Cmd + E)"
      aria-label={`Export ${transactions.length} transaction${transactions.length !== 1 ? 's' : ''} as CSV`}
    >
      <svg
        className="w-4 h-4"
        fill="none"
